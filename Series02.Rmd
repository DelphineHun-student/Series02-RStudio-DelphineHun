...
**Test?**
Before choosing the appropriate statistical test, we first need to assess whether the differences between before and after measurements follow a normal distribution. We used the Shapiro–Wilk test (Shapiro & Wilk, 1965) to assess whether the distribution of differences between before and after measurements could be considered normal, since the choice between a paired t-test and a Wilcoxon signed-rank test depends on this assumption
```{r Shapiro-Wilk test}
diff <- prepost$perf[prepost$time=="After"] - prepost$perf[prepost$time=="Before"]
shapiro.test(diff)

hist(diff, main="Histogram of differences", xlab="After - Before")
